...
class FeedManager
  ...
  # @param [Boolean] update
  # @return [Boolean]
  def unpush_from_home(account, status, update: false, mark_expired: false)
    return false unless remove_from_feed(:home, account.id, status, account.user&.aggregates_reblogs?)

    unless update
      redis.publish("timeline:#{account.id}", Oj.dump(event: mark_expired ? :expire : :delete, payload: status.id.to_s))
    end
    true
  end
  ...
  # @param [Boolean] update
  # @return [Boolean]
  def unpush_from_list(list, status, update: false, mark_expired: false)
    return false unless remove_from_feed(:list, list.id, status, list.account.user&.aggregates_reblogs?)

    unless update
      redis.publish("timeline:list:#{list.id}", Oj.dump(event: mark_expired ? :expire : :delete, payload: status.id.to_s))
    end
    true
  end
