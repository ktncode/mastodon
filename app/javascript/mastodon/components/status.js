...
const messages = defineMessages({
  ...
  mark_ancestor: {
    id: "thread_mark.ancestor",
    defaultMessage: "Has reference",
  },
  mark_descendant: {
    id: "thread_mark.descendant",
    defaultMessage: "Has reply",
  },
  mark_both: {
    id: "thread_mark.both",
    defaultMessage: "Has reference and reply",
  },
...
class Status extends ImmutablePureComponent {
  ...
  state = {
    ...
    showQuoteMedia: defaultMediaVisibility(
      this.props.status ? this.props.status.get("quote", null) : null
    ),
    statusId: undefined,
  };
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (nextProps.status && nextProps.status.get("id") !== prevState.statusId) {
      return {
        ...
        showQuoteMedia: defaultMediaVisibility(
          nextProps.status.get("quote", null)
        ),
  ...
  handleEmojiClick = (e) => {
    ...
    if (this.context.router) {
      ...
      this.context.router.history.push(
        `/emoji_detail/${shortcode}${domain ? `@${domain}` : ""}`
      );
    }
  };
  ...
  handleClick = () => {
    ...
    this.context.router.history.push(
      `/statuses/${status.getIn(["reblog", "id"], status.get("id"))}`
    );
  };
  ...
  handleQuoteClick = () => {
    ...
    const { status } = this.props;
    this.context.router.history.push(
      `/statuses/${status.getIn(
        ["reblog", "quote", "id"],
        status.getIn(["quote", "id"])
      )}`
    );
  };
  ...
  handleHotkeyOpen = () => {
    this.context.router.history.push(
      `/statuses/${this._properStatus().get("id")}`
    );
  };

  handleHotkeyOpenProfile = () => {
    this.context.router.history.push(
      `/accounts/${this._properStatus().getIn(["account", "id"])}`
    );
  };
  ...
  _properQuoteStatus() {
    ...
    if (
      status.get("quote", null) !== null &&
      typeof status.get("quote") === "object"
    ) {
  ...
  render() {
    ...
    if (
      status.get("filtered", []).length ||
      status.getIn(["reblog", "filtered"], []).length
    ) {
    ...
    if (featured) {
      ...
      if (!hideRebloggedBy) {
        ...
        const visibilityReblogIcon =
          visibilityIconInfo[status.get("visibility")];
        const visibilityReblogLink = (
          <Icon
            id={visibilityReblogIcon.icon}
            className="status__prepend-icon"
            title={visibilityReblogIcon.text}
          />
        );
    ...
    if (status.get("media_attachments").size > 0) {
      if (pictureInPicture.get("inUse")) {
        ...
        media = (
          <Bundle
            fetchComponent={ThumbnailGallery}
            loading={this.renderLoadingThumbnailGallery}
          >
            {(Component) => (
              <Component
    ...
    if (
      ...
      if (!showCard) {
        ...
        if (pictureInPicture.get("inUse")) {
          ...
          quote_status.getIn(["media_attachments", 0, "type"]) === "audio"
        ) {
            ...
            >
              {(Component) => (
                <Component
                  ...
                  accentColor={attachment.getIn(["meta", "colors", "accent"])}
                  duration={attachment.getIn(
                    ["meta", "original", "duration"],
                    0
                  )}
                  width={this.props.cachedMediaWidth}
                  height={70}
      ...
      if (quote_muted) {
        quote = (
          <div
            className={classNames(
              "quote-status",
              `status-${quote_status.get("visibility")}`,
        ...
        quote_status.get("visibility") === "unlisted" &&
        !!contextType &&
        ["public", "community", "hashtag"].includes(
          contextType.split(":", 2)[0]
        )
      ) {
        quote = (
          <div
          ...
          >
            <div
              className={classNames("status__content unlisted-quote", {
                "status__content--with-action": this.context.router,
              })}
            >
          ...
          >
            <div className="status__info">
              <a
                ...
                title={quote_status.getIn(["account", "acct"])}
    ...
    const ancestorCount =
      showThread &&
      show_reply_tree_button &&
      status.get("in_reply_to_id", 0) > 0
        ? 1
        : 0;
    const descendantCount =
      showThread && show_reply_tree_button ? status.get("replies_count", 0) : 0;
    const referenceCount = enableStatusReference
      ? status.get("status_references_count", 0) -
        (status
          .get("status_reference_ids", ImmutableList())
          .includes(status.get("quote_id"))
          ? 1
          : 0)
      : 0;
          ...
          <div
            className={classNames(
              ...
              `status-${status.get("visibility")}`,
              {
                ...
                muted: this.props.muted,
                "status-with-expiration": expires_date,
          ...
          >
            ...
            <div className="status__info">
              ...
              >
                {threadMark}
                {disableRelativeTime ? (
                  <AbsoluteTimestamp timestamp={status.get("created_at")} />
                ) : (
            ...
            {enableReaction &&
              (contextType === "thread" || !compactReaction) && (
                <EmojiReactionsBar
                  status={status}
