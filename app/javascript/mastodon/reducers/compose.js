...
const statusToTextMentions = (text, privacy, replyStatus) => {
  ...
  const replyStatusMentions = replyStatus
    .get("mentions")
    .filterNot((mention) => mention.get("id") === me);
  const groupMentions = replyStatusMentions.filter((mention) =>
    mention.get("group")
  );
  const otherMentions = replyStatusMentions.filterNot((mention) =>
    mention.get("group")
  );

  mentions = mentions.union(
    groupMentions.map(
      (mention) => `@${mention.get("moved_acct") ?? mention.get("acct")} `
    )
  );
  ...
  if (replyStatus.getIn(["account", "id"]) !== me) {
    mentions = mentions.add(
      `@${
        replyStatus.getIn(["account", "moved_acct"]) ??
        replyStatus.getIn(["account", "acct"])
      } `
    );
  }
...
const appendMedia = (state, media, file) => {
  ...
  return state.withMutations((map) => {
    ...
    map.update("media_attachments", (list) =>
      list.push(media).sortBy((media) => media.get("order"))
    );
...
const hydrate = (state, hydratedState) => {
  ...
  if (hydratedState.has("prohibited_visibilities")) {
    state = state.set(
      ...
      hydratedState.get("prohibited_visibilities").toSet()
    );
  }
  ...
  if (hydratedState.has("prohibited_words")) {
    state = state.set(
      "prohibited_words",
      hydratedState.get("prohibited_words").toSet()
    );
  }
...
const stripCompatibleText = (html) => {
  ...
  const original_media_link = fragment.querySelector(
    "span.original-media-link"
  );
...
export default function compose(state = initialState, action) {
  switch (action.type) {
    ...
    case COMPOSE_VISIBILITY_CHANGE:
      return state.withMutations((map) => {
        const searchability = searchabilityCap(
          action.value,
          state.get("searchability")
        );
    ...
    case COMPOSE_REPLY:
      return state.withMutations((map) => {
        const privacy = privacyCap(
          action.status.get("visibility"),
          state.get("default_privacy")
        );
        const searchability = searchabilityCap(
          action.status.get("visibility"),
          state.get("default_searchability")
        );
        ...
        map.set(
          "default_expires",
          state.get("default_expires_in") ? true : null
        );
        map.set("scheduled", null);
        map.set("expires", state.get("default_expires_in", null));
        map.set("expires_action", state.get("default_expires_action", "mark"));
        map.update("context_references", (set) =>
          set.clear().concat(action.context_references)
        );
        map.set("scheduled_status_id", null);
    ...
    case COMPOSE_QUOTE:
      return state.withMutations((map) => {
        ...
        map.set("expires_action", state.get("default_expires_action", "mark"));
        map.update("context_references", (set) =>
          set.clear().add(action.status.get("id"))
        );
        map.set("scheduled_status_id", null);
    ...
    case COMPOSE_RESET:
      return state.withMutations((map) => {
        ...
        map.update("context_references", (set) => set.clear());
        if (
          action.type === COMPOSE_RESET ||
          action.type === COMPOSE_SCHEDULED_EDIT_CANCEL
        ) {
          map.update("references", (set) => set.clear());
        }
        map.set("scheduled_status_id", null);
    ...
    case REDRAFT:
      return state.withMutations((map) => {
        const datetime_form =
          !!action.status.get("scheduled_at") ||
          !!action.status.get("expires_at")
            ? true
            : null;
        ...
        map.set(
          "text",
          action.raw_text ||
            unescapeHTML(stripCompatibleText(expandMentions(action.status)))
        );
        ...
        map.set(
          "privacy",
          action.status.get("visibility", state.get("default_privacy"))
        );
        map.set(
          "searchability",
          action.status.get("searchability", state.get("default_searchability"))
        );
        map.set("circle_id", action.status.get("circle_id", null));
        map.set(
          "media_attachments",
          action.status.get("media_attachments", ImmutableList())
        );
        map.set("focusDate", new Date());
        map.set("caretPosition", null);
        map.set("idempotencyKey", uuid());
        ...
        map.set(
          "default_expires",
          !datetime_form && state.get("default_expires_in") ? true : null
        );
        map.set(
          "scheduled",
          action.status.get("scheduled_at")
            ? format(
                new Date(action.status.get("scheduled_at")),
                "yyyy-MM-dd HH:mm"
              )
            : null
        );
        map.set(
          ...
          action.status.get("expires_at")
            ? format(
                new Date(action.status.get("expires_at")),
                "yyyy-MM-dd HH:mm"
              )
            : state.get("default_expires_in", null)
        );
        map.set(
          "expires_action",
          action.status.get("expires_action") ??
            state.get("default_expires_action", "mark")
        );
        map.update("references", (set) =>
          set
            .clear()
            .concat(action.status.get("status_reference_ids", ImmutableList()))
            .delete(action.status.getIn(["quote", "id"], ImmutableList()))
        );
        map.update("context_references", (set) =>
          set.clear().concat(action.context_references)
        );
        ...
        map.set(
          "scheduled_status_id",
          action.status.get("scheduled_status_id", null)
        );
          ...
          map.set(
            ...
            ImmutableMap({
              options: action.status
                .getIn(["poll", "options"])
                .map((x) => (typeof x === "string" ? x : x.get("title"))),
