...
class EmojiDetail extends React.PureComponent {
  ...
  render () {
    const { intl, params: { shortcode_with_domain }, custom_emoji, isLoading, multiColumn, columnWidth } = this.props;
    ...
    if (!custom_emoji) {
      if (isLoading) {
        ...
        return (
          <Column>
            <FormattedMessage id='empty_column.custom_emoji_detail' defaultMessage="There are no matching custom emojis" />
          </Column>
        );
      }
    }
      ...
      <Column bindToDocument={!multiColumn} label={intl.formatMessage(messages.heading)} columnWidth={columnWidth}>
        ...
        <div className='custom-emoji__detail' ref={this.setRef}>
          <div className='custom-emoji__image' ref={this.setTargetRef}>
            <Emoji emoji={shortcode_with_domain} hovered={this.state.hovered} url={custom_emoji.get('url')} static_url={custom_emoji.get('static_url')} />
          </div>
