...
const messages = defineMessages({
  ...
  emoji_reaction_disable: {
    id: "status.emoji_reaction_disable",
    defaultMessage: "Emoji reaction",
  },
  emoji_reaction_expired: {
    id: "status.emoji_reaction_expired",
    defaultMessage: "Emoji reaction",
  },
  emoji_reaction_limit_reatched: {
    id: "status.emoji_reaction_limit_reatched",
    defaultMessage: "Emoji reaction",
  },
  show_reblogs: {
    id: "status.show_reblogs",
    defaultMessage: "Show boosted users",
  },
  show_favourites: {
    id: "status.show_favourites",
    defaultMessage: "Show favourited users",
  },
  show_emoji_reactions: {
    id: "status.show_emoji_reactions",
    defaultMessage: "Show emoji reactioned users",
  },
  show_referred_by_statuses: {
    id: "status.show_referred_by_statuses",
    defaultMessage: "Show referred by statuses",
  },
  ...
  openDomainTimeline: {
    id: "account.open_domain_timeline",
    defaultMessage: "Open {domain} timeline",
  },
  unmute: { id: "account.unmute", defaultMessage: "Unmute @{name}" },
  unblock: { id: "account.unblock", defaultMessage: "Unblock @{name}" },
  visibilityMatchMessage: {
    id: "visibility.match_message",
    defaultMessage:
      "Do you want to match the visibility of the post to the reference?",
  },
  visibilityKeepMessage: {
    id: "visibility.keep_message",
    defaultMessage:
      "Do you want to keep the visibility of the post to the reference?",
  },
...
const mapStateToProps = (state, { status }) => ({
  ...
  referenceCountLimit:
    state.getIn(["compose", "references"]).size >= maxReferences,
...
class ActionBar extends React.PureComponent {
  ...
  handleReferenceClick = (e) => {
    ...
    if (selected) {
      ...
      if (
        ...
        ["public", "unlisted"].includes(composePrivacy)
      ) {
          ...
          dispatch(
            openModal("CONFIRM", {
              message: intl.formatMessage(
                matchVisibilityOfReferences
                  ? messages.visibilityMatchMessage
                  : messages.visibilityKeepMessage
              ),
              confirm: intl.formatMessage(
                matchVisibilityOfReferences
                  ? messages.visibilityChange
                  : messages.visibilityKeep
              ),
              ...
              onSecondary: () =>
                onAddReference(id, !matchVisibilityOfReferences),
            })
          );
        }
      } else {
  ...
  handleOpenDomainTimeline = () => {
    ...
    this.context.router.history.push(
      `/timelines/public/domain/${account.get("acct").split("@")[1]}`
    );
  };
  ...
  handleReblogs = () => {
    this.context.router.history.push(
      `/statuses/${this.props.status.get("id")}/reblogs`
    );
  };

  handleFavourites = () => {
    this.context.router.history.push(
      `/statuses/${this.props.status.get("id")}/favourites`
    );
  };

  handleEmojiReactions = () => {
    this.context.router.history.push(
      `/statuses/${this.props.status.get("id")}/emoji_reactions`
    );
  };

  handleReferredByStatuses = () => {
    this.context.router.history.push(
      `/statuses/${this.props.status.get("id")}/referred_by`
    );
  };
  ...
  render() {
    ...
    const showEmojiReactionCount =
      !hideListOfEmojiReactionsToPosts &&
      enableReaction &&
      !status.get("emoji_reactions").isEmpty();
    ...
    if (
      showReblogCount ||
      showFavouritCount ||
      showEmojiReactionCount ||
      showStatusReferredByCount
    ) {
      menu.push(null);
    }
    ...
    if (showReblogCount) {
      menu.push({
        text: intl.formatMessage(messages.show_reblogs),
        action: this.handleReblogs,
      });
    }

    if (showFavouritCount) {
      menu.push({
        text: intl.formatMessage(messages.show_favourites),
    ...
    if (showEmojiReactionCount) {
      menu.push({
        text: intl.formatMessage(messages.show_emoji_reactions),
        action: this.handleEmojiReactions,
      });
    }

    if (showStatusReferredByCount) {
      menu.push({
        text: intl.formatMessage(messages.show_referred_by_statuses),
        action: this.handleReferredByStatuses,
      });
    }
    ...
    if (domain) {
      menu.push(null);
      menu.push({
        text: intl.formatMessage(messages.openDomainTimeline, { domain }),
        action: this.handleOpenDomainTimeline,
      });
    }
    ...
    const referenceDisabled =
      expired ||
      (!referenced && referenceCountLimit) ||
      ["limited", "direct", "personal"].includes(status.get("visibility"));
      ...
      <div className="detailed-status__action-bar">
        ...
        {enableStatusReference && me && (
          <div className="detailed-status__button">
            <IconButton
              ...
              animate
              disabled={disablePost || referenceDisabled}
              active={referenced}
              pressed={referenced}
        ...
        <div className="detailed-status__button">
          <IconButton
            ...
            disabled={
              disableReactions || (!publicStatus && !reblogPrivate) || expired
            }
            active={reblogged}
            title={reblogTitle}
            icon="retweet"
        ...
        <div className="detailed-status__button">
          <IconButton
            ...
            animate
            active={favourited}
            disabled={disableReactions || (!favourited && expired)}
        ...
        {show_quote_button && (
          <div className="detailed-status__button">
            <IconButton
              disabled={disablePost || !publicStatus || expired}
              title={
                !publicStatus
                  ? intl.formatMessage(messages.cannot_quote)
