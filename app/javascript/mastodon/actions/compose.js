...
export const SCHEDULED_STATUS_SUBMIT_SUCCESS =
...
const messages = defineMessages({
  ...
  postReferenceMessage: {
    id: "confirmations.post_reference.message",
    defaultMessage: "It contains references, do you want to post it?",
  },
  postReferenceConfirm: {
    id: "confirmations.post_reference.confirm",
    defaultMessage: "Post",
  },
  missingAltTextTitle: {
...
export const ensureComposeIsVisible = (getState, routerHistory) => {
  if (
    !getState().getIn(["compose", "mounted"]) &&
...
export const getContextReference = (getState, status) => {
  ...
  const replyStatus = status.get("in_reply_to_id")
    ? getState().getIn(["statuses", status.get("in_reply_to_id")])
    : null;
...
const parseSimpleDurationFormat = (value, origin = new Date()) => {
  ...
  const [, year = 0, month = 0, day = 0, hour = 0, minute = 0] =
    value.match(
      /^(?:(\d+)y)?(?:(\d+)m(?=[\do])o?)?(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?$/
    ) ?? [];
  const duration = millisecondsToSeconds(
    addMinutes(
      addHours(addDays(addMonths(addYears(origin, year), month), day), hour),
      minute
    ) - origin
  );
...
export function submitComposeWithCheck(routerHistory, intl) {
  return function (dispatch, getState) {
    ...
    const missingAltTextMediaId = media
      .find(
        (media) =>
    ...
    const ignoreStatusReferenceCheck = getState().getIn([
      "compose",
      "ignore_reference_check",
    ]);
    ...
    if (
      postReferenceModal &&
      !statusReferenceIds.isEmpty() &&
      !ignoreStatusReferenceCheck
    ) {
      dispatch(
        openModal("CONFIRM", {
          ...
          onConfirm: () =>
            dispatch(submitComposeIgnoreReferenceCheck(routerHistory)),
        })
      );
...
export function submitCompose(routerHistory) {
  return function (dispatch, getState) {
    ...
    const { in: scheduled_in = null, at: scheduled_at = null } =
      getDateTimeFromText(
        getState().getIn(["compose", "scheduled"]),
        new Date()
      );
    const { in: expires_in = null, at: expires_at = null } =
      getDateTimeFromText(
        getState().getIn(["compose", "expires"]),
        scheduled_at ?? new Date()
      );
    const expires_action = getState().getIn(["compose", "expires_action"]);
    const statusReferenceIds = getState().getIn(["compose", "references"]);
    const scheduled_status_id = getState().getIn([
      "compose",
      "scheduled_status_id",
    ]);
    ...
    if (statusId === null) {
      Object.assign(requestData, {
        ...
        scheduled_at:
          !scheduled_in && scheduled_at
            ? formatISO(set(scheduled_at, { seconds: 0 }))
            : null,
        scheduled_in: scheduled_in,
        expires_at:
          !expires_in && expires_at
            ? formatISO(set(expires_at, { seconds: 59 }))
            : null,
    ...
    api(getState)
      ...
      .then(function (response) {
        if (
          response.data.scheduled_at !== null &&
          response.data.scheduled_at !== undefined
        ) {
        ...
        if (statusId === null) {
          if (
            homeVisibilities.length ||
            homeVisibilities.includes(response.data.visibility)
          ) {
            insertIfOnline("home");
          }
    ...
    for (const [i, file] of Array.from(files).entries()) {
      ...
      api(getState)
        ...
        .then(({ status, data }) => {
          ...
          if (status === 200) {
            dispatch(
              uploadComposeSuccess({ ...data, order: orderBase + i }, file)
            );
          } else if (status === 202) {
                  ...
                  if (response.status === 200) {
                    dispatch(
                      uploadComposeSuccess(
                        { ...response.data, order: orderBase + i },
                        file
                      )
                    );
...
function insertIntoTagHistory(recognizedTags, text) {
  return (dispatch, getState) => {
    ...
    const names = recognizedTags.map((tag) =>
      text.match(new RegExp(`#${tag.name}`, "i"))[0].slice(1)
    );
...
export function changeScheduled(value) {
  const date =
    value instanceof Date ? format(value, "yyyy-MM-dd HH:mm") : value;
